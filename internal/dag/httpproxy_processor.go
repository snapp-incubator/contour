...
func (p *HTTPProxyProcessor) computeHTTPProxy(proxy *contour_v1.HTTPProxy) {
	...
	if tls := proxy.Spec.VirtualHost.TLS; tls != nil {
		...
		if !tls.Passthrough {
			...
			if tls.EnableFallbackCertificate && len(proxy.Spec.VirtualHost.AuthorizationProviders) > 0 {
				validCond.AddError(contour_v1.ConditionTypeTLSError, "TLSIncompatibleFeatures",
			...
			for _, route := range proxy.Spec.Routes {
				...
				for _, provider := range proxy.Spec.VirtualHost.AuthorizationProviders {
			...
			if len(proxy.Spec.VirtualHost.AuthorizationProviders) > 0 {
				if proxy.Spec.VirtualHost.TLS == nil || len(proxy.Spec.VirtualHost.TLS.SecretName) == 0 {
					validCond.AddError(contour_v1.ConditionTypeAuthError, "AuthNotPermitted",
				...
				for _, provider := range proxy.Spec.VirtualHost.AuthorizationProviders {
					...
					svhost.AuthorizationProviders = append(svhost.AuthorizationProviders, AuthorizationProvider{
						Name:                         provider.Name,
						Default:                      provider.Default,
						ExtensionCluster:             extensionSvc,
						ServiceType:                  provider.ServiceType,
						PathPrefix:                   provider.HTTPServerSettings.PathPrefix,
						HeadersToAdd:                 provider.HTTPServerSettings.HeadersToAdd,
						AllowedAuthorizationHeaders:  provider.HTTPServerSettings.AllowedAuthorizationHeaders,
						AllowedUpstreamHeaders:       provider.HTTPServerSettings.AllowedUpstreamHeaders,
...
func (p *HTTPProxyProcessor) computeRoutes(
	...
	for _, route := range proxy.Spec.Routes {
		...
		if vhostAuthorization != nil && vhostAuthorization.AuthPolicy != nil && !vhostAuthorization.AuthPolicy.Disabled {
			authDisabled = false
		}
		if defaultAuthzProvider != nil {
			authDisabled = false
		}
		if routeAuthPolicy != nil {
			if len(routeAuthPolicy.Require) > 0 {
				authDisabled = false
			} else if routeAuthPolicy.Disabled {
				authDisabled = true
			}
		}
		...
		if authDisabled {
			...
			if len(route.AuthPolicy.Require) > 0 {
				for _, provider := range rootProxy.Spec.VirtualHost.AuthorizationProviders {
					if provider.Name == route.AuthPolicy.Require {
